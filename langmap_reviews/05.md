...
## 包括スイープ — 群05 全複合セル一括判定 (作業者 round 9)

対象言語: yue, nan, hak_cn, cdo, ii, za, hmn。/tmp/langmap_sweep/05.md の全複合セルを live (.wf_langmap_query.mjs) で検証し判定。

SPLIT: 36 件 / KEEP: 18 件。

### SPLIT の要点
- #35 ii: A|E ꉢ(我)/ꀊꈌ(兄), C|D ꃅꃶ(上個)/ꆪ(月)。兄弟全変種が分割。
- #38 ii: A|D → D:ꀉꎆ(here)/A:ꇬ(後置詞 from)。
- #46 nan/hak_cn/cdo/ii/za: E|H 動詞+完了アスペクト(講+了 / ꌧ+ꌐ / gangj+le)。yue が既に傾/咗で分割 → fix-all。hmn(tham)はアスペクト無しで KEEP。
- #50 ii: A|E → E:ꉢꈬ(我个)/A:ꁡ(狗)。所有+名詞で連続。
- #51 hak_cn/ii/za/hmn: A|E 所有+祖母(𠊎+阿婆 / ꉢ+ꀊꂾꀉꂾ / Gou+daiqmeh / Kuv+pog)。所有代名詞が前置で連続=分割可。nan(阿媽)・cdo(阿嬤)は所有トークン無しで KEEP。
- #52 ii: C|H 動詞+完了(ꃬ+ꌐ)、D|F 市場+処格(ꇬꂷ+ꃅ)。
- #53 ii: A|D 庭+処格(ꑤꃅ+ꄉ)、C|E 開+進行(ꇈ+ꑌ)。
- #55 ii: B|E|F → B:ꈩꐱ(咖啡)/E:ꇖ(両)/F:ꊏ(杯)。
- #69 yue/nan/ii: C|H → 与格+両親(畀+佢父母 / 予+父母 / ꀊꃀꀉꂾ+ꈬ)。hak_cn/cdo/za/hmn は既に分割済 → fix-all。D|E 動詞+完了は yue寫咗/nan寫?/hak_cn寫了/za sij le/ii ꏂꌐ のうちトークン2個のもの(yue/hak_cn/za/ii)を分割。nan「寫」・cdo「寫」・hmn「sau」は単一語で KEEP。
- #73 nan/cdo: C|G 毎+暝(每+暝)。za: C|G haemh/haemh(畳語、役割は別IDなので可)。ii: A|E(ꉢꈬ/ꃬꏢ)・B|F(ꉢꈬꋠ/ꃅ 床+上)。
- #77 ii/za: za E:gaujsi/B:gij gyet guoj(test/results、Dは無表現)。ii は分離可能な後置詞 D:ꈬ のみ分離(B|E:ꈚꏂꊌ は融合維持)。hmn(qhov kev xeem)は results を単独切出し不可で KEEP。
- #80 hak_cn: B|F → F:出國/B:留學(出國が明示)。yue/nan/cdo「留學」は abroad を含む単一複合名詞、ii は融合不確定で KEEP。
- #84 yue/nan/hak_cn/cdo/ii/hmn: D|E 先週(上個/星期 等、demonstrative+名詞で連続)。B|C は yue傷風/咗・nan/cdo傷風/去 を分割。za「laeng ndeu baez ngoenz lai」は5トークンの慣用句で境界不確定 → D|E KEEP。
- #89 yue/nan/hak_cn/cdo: D|C 動詞+否定(搵/唔到 等)。hmn が既に分割 → fix-all。

### KEEP の根拠(分離不可・無表現・不確定)
- #35/#43/#45/#46/#48/#49/#55/#70/#73/#77/#80/#84/#85/#89 の ii 等で、彝文(Yi)の音節ブロックが3役以上に融合し、辞書的に各形態素を役割へ確実に割当できないもの(#45 A|E|F, #49 C|E|F|G, #70 B|D|E|F, #48 B|E・C|D, #85 ii B|E, #89 ii/za D|C)は誤割当回避のため KEEP。
- #43 A|E(餐廳/食堂/gya haeux dien/Lub tsev noj mov): 前置詞「at」に対応する分離トークンが無く、レストランの単一名詞が処格を吸収(無表現) → KEEP。
- #46 B|G(朋友/phooj ywg 等): 所有「my」の分離トークンが無い(無表現) → KEEP。
