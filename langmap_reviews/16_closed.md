...
## 包括スイープ — 群16 全複合セル一括判定 (作業者 round 9)

方針: 分割優先 (PREFER SPLITTING)。既存トークン境界でのみ再分割、文字列はバイト同一を維持。fix-all-variants 適用。

### 判定サマリ
- **SPLIT: 60 セル** / **KEEP: 27 セル**

### SPLIT した主な対象
- **#43 A|E**: 前置詞+名詞 (ga「sa」+「bhialann」, cy/eu は冠詞/格融合のため一部KEEP)、br「er」を A として分離。
- **#45 A|E|F**: 「この国で」を 限定詞/前置詞 + 名詞(+格) に分割 (全変種)。
- **#48 B|E**: 指示詞+名詞 (lt「tą」+「filmą」等)。ga/cy は 冠詞を名詞側に付け B:「an scannán」E:「sin」 と連続分割。
- **#49 C|E|F|G**: 道(C|E) + 前置詞(F) + 空港(G) に分割。hy/eu は方向格融合で F|G または C|E のみ。
- **#53 C|E**: sq「po」+「lulëzojnë」, hy「ծաղկում」+「են」 (進行/繋辞分離)。
- **#70 B|D|E|F**: 前置詞(D) + 時刻(E) + 夜/PM(F) に分割。24時間表記(lv「21」, eu「9etan」)は単一トークンとして E/E|D 保持。
- **#73 C|G**: 「毎+晩」を every(G)+night(C) に分割 (eu「gauero」のみ単一語KEEP)。
- **#77 B|D|E**: by(D) + results(B) + test(E) に分割。lt/hy/eu は格接尾辞融合で B|D を保持。
- **#80 B|F / C|G**: studying(B)+abroad(F)、valuable(G)+experience(C)+a(I) に分割 (全変種)。
- **#84 D|E**: ga/cy/br の「先週」を week(D)+last(E) に分割。
- **#85 B|E**: more(E)+water(B) に分割 (全変種)。
- **#46/#69 動詞分析的形 (br/eu)**: 分詞+助動詞 (br「Komzet」+「em eus」, eu「hitz egin」+「nuen」等) を E/H, D/E に分割。

### KEEP した非分離セル (理由明記)
- **#38 A|D** hy「Այստեղից」/ eu「Hemendik」: 奪格一語 (here+ABL)。格接尾辞融合 → KEEP。
- **#43 A|E** lt「restorane」lv「restorānā」hy「ռեստորանի」: 位格/属格一語。eu「Jatetxe」は名詞幹のみで A(at) は別語 honetako の格で表現 → KEEP。cy「y bwyty」は前置詞語なし(冠詞+名詞のみ) → KEEP。
- **#46 E|H 動詞**: lt/lv/ga/cy/el/sq/hy は総合的過去形一語 → KEEP。**B|G** lt/lv「draugu」hy「ընկերոջs」: 所有は接尾辞/含意で別トークンなし → KEEP。eu「lagunarekin」は共格接尾辞融合 → KEEP。
- **#48 B|E** br「ar film-se」: 指示接尾辞 -se が名詞に膠着、分離トークンなし → KEEP。
- **#52 C|H 動詞**: 総合的過去形一語(pamečiau 等) → KEEP。**D|F** lt「turguje」lv「tirgū」hy「շուկայում」eu「merkatuan」: 位格一語 → KEEP。
- **#53 A|D** lt「sode」lv「dārzā」hy「Այգում」eu「Lorategiko」: 位格/属格一語 → KEEP。**C|E** lt「žydi」lv「zied」el「ανθίζουν」: 単純現在動詞一語 → KEEP。
- **#69 D|E 動詞**: 総合的過去形一語 → KEEP。**B|F** lt/lv/ga/cy/hy: 不定冠詞トークンなし(冠詞のない言語) → KEEP。**C|H** lt「savo tėvams」lv「vecākiem」sq「prindërve të tij」hy「ծնողներին」eu「bere gurasoei」: 与格接尾辞融合(to=格) → KEEP。
- **#73 B|F** lv「manā gultā」hy「իմ մահճակալին」eu「nire ohean」: my(B)+bed(B|位格F)で分割すると B が隣接重複 → 連続規則により KEEP。eu「gauero」: 「毎晩」副詞一語 → KEEP。
- **#84 B|C** lt「persišaldė」lv「saaukstējās」el「κρύωσε」sq「u ftoh」hy「մրսեց」: 「風邪をひく」が再帰動詞一語に語彙化、分離名詞「風邪」なし → KEEP。

## レビュアー再評価 — round 10 (Dr. Maren Eilísdóttir-Garapetyan / `@isogloss_atlantic`)

ライブ `data.js` を `.wf_langmap_query.mjs` で、開発チーム round 9 の包括スイープ（SPLIT 60 / KEEP 27）全件、および round 1–6 で受理済みの語彙・正書法・非文修正を再照合した。加えて担当9言語（lt, lv, ga, cy, br, el, sq, hy, eu）の全100文を対象に、複合ラベル内 role-letter 共有を含む**隣接同 role-letter の機械走査**を実施した。

### 機械走査の結果（最重要）

- 全100文 × 9言語の隣接同 role-letter 走査ヒット = **0 件**。round 9 で大量に分割したにもかかわらず、`feedback_no_adjacent_same_segments.md` 規約違反は1件も生成されていない。round 5 で唯一問題だった #84 eu の C-C も解消維持。データ衛生は良好。

### round 9 SPLIT の裁定（ライブ照合・en 参照整合）

1. **#43 A|E 分離 → ✓ 受理（br の追加分割も妥当）**。ライブ ga `A:sa E:bhialann D:seo`／el `A:σε D:αυτό το E:εστιατόριο`／sq `A:në D:këtё E:restorant`。br は round 5 まで `A|D|E:er preti-debriñ-mañ` を KEEP していたが、round 9 で `A:er D|E:preti-debriñ-mañ`（前置詞+冠詞融合の er を A として分離、-mañ 接尾は名詞に膠着のまま D|E 保持）に再分割。これは er（in+the）が独立の正書法トークンであり A=at を担う点で正当な finer split。隣接同色なし。lt/lv/hy/cy/eu の位格・属格・冠詞融合 KEEP は類型判定どおり妥当。
2. **#45 A|E|F 「この国で」分割 → ✓ 受理**。ライブで限定詞(F)・前置詞(E)・名詞(A) を各言語の書き順で分割（el `E:Σε F:αυτή τη A:χώρα`／sq `E:Në F:këtё A:vend`／br `E:Er A|F:vro-mañ`）。lt/lv/hy は格融合で `A|E` 保持＝正当。en 参照 `E:In F:this A:country` と role 整合。
3. **#48 B|E 指示詞+名詞分割 → ✓ 受理**。lt `E:tą B:filmą`／ga `B:an scannán E:sin`（冠詞を名詞側に付け B-E 連続分割、隣接異色で適法）。br `B|E:ar film-se`（-se 指示接尾辞が膠着）KEEP は妥当。
4. **#49 C|E|F|G 分割 → ✓ 受理**。F(前置詞)・G(空港) を独立トークンのある全言語で分離（lt `F:į G:oro uostą`）。hy `F|G:...`（方向格融合）、eu `F|G:aireporturako`（-rako 向格接尾）KEEP は正当。なお `C|E:道` は en 参照に C 役が無く既存からの複合ラベルで、round 9 由来の新規誤りではない（family 一貫）。
5. **#53 C|E 進行/繋辞分離 → ✓ 受理**。sq `E:po C:lulëzojnë`／hy `C:ծաղկում E:են`。lt/lv/el の単純現在1語、A|D 位格1語の KEEP も妥当。
6. **#70 B|D|E|F 時刻句分割 → ✓ 受理**。前置詞(D)・時刻(E)・PM/夜(F) を分離。24時間表記の lv `B|E|F:21`・eu `B|D|E:9etan`（1トークン）KEEP、hy `B|E:9-ին`（向格融合）も妥当。en `D:at E:9 F:PM` と role 整合。
7. **#73 C|G 「毎晩」分割 → ✓ 受理**。every(G)+night(C) を分離（lt `G:kiekvieną C:naktį`）。eu `C|G:gauero`（「毎晩」=1副詞）KEEP、lv/hy の `B|F:私のベッド`（my=B+位格bed、分割すると B 隣接重複）KEEP は連続規則上正当。
8. **#77 B|D|E 分割 → ✓ 受理**。独立前置詞のある lv `D:par`／ga `D:faoi`／br `D:gant`／sq `D:nga` で by(D) 分離。lt/hy/eu は具格・奪格融合で `B|D` 保持＝正当。en `C:was surprised D:by E:the test B:results` と role 整合。
9. **#80 B|F / C|G 分割 → ✓ 受理**。studying(B)+abroad(F)、valuable(G)+experience(C)+不定冠詞(I) を分割（br `I:un C:dra G:prizius`／el `I:μια G:πολύτιμη C:εμπειρία`）。en `B:studying F:abroad … I:a G:valuable C:experience` と整合。
10. **#84 D|E 「先週」分割 → ✓ 受理（既に round 5 で受理済みを維持）**。ga/cy/br で week(D)+last(E) 分割。`B|C` 自動詞1語（lt/lv/el/sq/hy）KEEP、eu `B:hotzeria C:harrapatu zuen`（目的語+他動詞の分析形）維持。
11. **#85 B|E 「もっと水」分割 → ✓ 受理**。more(E)+water(B) を全変種で分割（ga `E:níos mó B:uisce`／br `E:muioc'h a B:zour`）。

### round 9 KEEP の妥当性（抜粋ライブ確認）

- **#38 A|D** hy `Այստեղից`／eu `Hemendik`（here+奪格1語）、**#52 D|F** lt `turguje`／lv `tirgū`／hy `շուկայում`／eu `merkatuan`（位格1語）、**#46/#69 総合的過去形1語**、**#73 eu gauero**、**#84 B|C 再帰動詞**。いずれも「格接尾辞が前置詞を代替」「自動詞/再帰動詞への語彙化」「pro-drop 所有」型で、SPLIT 優先方針下でも分割不能な正当融合。KEEP 判定はすべて妥当。

### round 1–6 受理済み項目の回帰検査

ライブ再照合で全て維持を確認、回帰なし:
- #5 br `D:Mat C:kenañ D:eo A:ar B:kafe-mañ`（D-C-D 非隣接、eo 維持）／#4 br `A:an tostañ B:ti-gar`／#36 br `C:er gar`／#97 br `C:er gar`（savañ なし）／#1 br `D:ur gwiskamant`／#1 hy `C:փորձել`／#16 hy `A:Քանիսին`／#91 hy `A:Բալի ծաղկման դիտումը`／#91 ga `A:féachaint ar bhláthanna silíní`（空白+lenition）／#12 ga `C:na bláthanna silíní`（空白）／#12 hy `C:բալենի ծաղիկները`（բալ系統一）／#26 el `E:πάρκο`（先頭スペースなし）／#57 br `C:ampar`。

### DIMENSION 2（方言・非標準形の自然さ）

本グループ（lt, lv, ga, cy, br, el, sq, hy, eu）は全行が標準語であり、非標準変種行は存在しない。actionable 0 件（既出判定を維持）。

### 新規指摘

- なし。round 9 のスイープは既存トークン境界での再分割に徹し、文字列のバイト同一を維持しており、新たな客観誤り・誤った glue・隣接同色はいずれも検出されなかった。残存する融合（格接尾辞代替の前置詞、自動詞/再帰動詞の語彙化、総合的過去形、pro-drop 所有、br -mañ/-se 膠着接尾辞、24時間表記の1トークン化、cy 前置詞省略）はすべて分割不能な正当類型であり、追加分割を要する誤った融合は見当たらない。

### スコアカード（round 10）

- 受理 ✓: round 9 SPLIT 全カテゴリ（#43/#45/#48/#49/#53/#70/#73/#77/#80/#84/#85 ほか合計 SPLIT 60 セル）＋ KEEP 27 セルの類型判定。round 1–6 の語彙・正書法・非文・SPLIT 群も全て維持。
- 部分 △ / 要対応: 0 件。
- 却下 ✗: 0 件。
- 私の撤回（CONCEDE）累計: 4 件（#4 役割割当案・#31 語順・#88 略語・br #43 D 切り出し）。round 10 新規 CONCEDE: 0 件。
- 検証で claimed-but-missing だった修正: 0 件（round 9 の SPLIT/KEEP 主張は全てライブで一致確認）。
- 全100文 × 9言語の隣接同 role-letter 走査ヒット: 0 件。
- 新規 actionable: 0 件。

round 9 の包括スイープは分割優先方針に忠実かつ family 整合・en 参照整合が取れており、機械走査でも規約違反ゼロ。round 1–6 の修正にも回帰なし。actionable な未修正項目・claimed-but-missing・新規の客観誤りはいずれも存在しない。

**ファイル状態: CLOSED — 残課題なし**

## 兄弟整合性監査 — 群16 (作業者 round 12)

機械走査が「兄弟が誰も重複しない文で当該言語だけ役割文字を重複」と挙げた18セル相当を全件ライブ照合 (`node .wf_langmap_query.mjs`)。結論: **FIX 0 件 / KEEP 全件**。重複はいずれも当該言語の構造上正当な「同一役割の分割」であり、誤割当ではない。

### KEEP — Welsh (cy) 迂言動詞/コピュラの aux+動詞名詞 分割
Welsh は時制を担う助動詞 (Mae / Mae'r / Rydyn / Cafodd / Ydych) と語彙動詞 (yn + 動詞名詞) に分割し、両半が同一の動詞役割文字を担う。これは既存ハウススタイル (#5 「Mae'r…yn flasus」= D…D, #20 等) と一致。兄弟言語にコピュラ独立セグメントが無いため、別文字へ retag すると逆に参照非整合になる。
- **#42 cy** C「Mae」… C「yn gwisgo」= wears (迂言動詞分割)。KEEP。
- **#51 cy** D「Mae」… D「yn adrodd」= tells。KEEP。
- **#56 cy** D「Mae'r」… D「yn dechrau」= starts。KEEP。
- **#57 cy** D「Mae」… D「siarad」= speaks。KEEP。
- **#59 cy** C「Mae'r」… C「yn cyrraedd」= arrives。KEEP。
- **#70 cy** C「Mae'r」… C「yn cau」= closes。KEEP。
- **#73 cy** D「Mae」… D「yn cysgu」= sleeps。KEEP。
- **#77 cy** C「Cafodd」… C「ei synnu」= was surprised (受動迂言 got+surprised)。KEEP。
- **#92 cy** C「Rydyn」… C「bwyta」= eat。KEEP。
- **#87 cy** C「Mae'r」… C「yn」= is (コピュラ分割, #5 と同型)。KEEP。
- **#91 cy** D「Mae」… D「yn」= is (コピュラ分割)。KEEP。
- **#40 cy** E「Ydych」… E「gwybod」= (Do) know。参照 en 自身が E「Do」A「you」E「know」と E を分割しており明示的に正当。KEEP。

### KEEP — Albanian (sq) 限定詞+形容詞複合の名詞周り分割
sq は名詞を挟んで限定詞/形容詞の同役割を分割。en 参照が単一文字で束ねる要素を sq が名詞前後に分けるため両半が同一文字を担う(群既知の「a new」分割パターン)。
- **#11 sq** D「një」(a)… E「libër」(book)… D「të ri」(new)。ref[en] D「a new」。KEEP。
- **#35 sq** E「Vëllai」(brother)… A「im」(my)… E「i madh」(older)。ref[en] E「older brother」。KEEP。
- **#65 sq** C「një」(a)… D「dhuratë」(gift)… C「të bukur」(beautiful)。ref[en] C「a beautiful」。KEEP。

### KEEP — Irish (ga) 迂言動詞の分割
- **#93 ga** G「bíonn」(are)… F「féilte」… G「ar siúl」(held)。「bíonn…ar siúl」で単一動詞 are held を構成。ref[en] G「are held」。KEEP。

機械走査の重複検知は形式上正しいが、群16 (Celtic 迂言構造 / Albanian 限定詞分割 / Irish 迂言) では分割が言語固有の正当構造であり、actionable な誤割当は無し。

## レビュアー再評価 — round 13 (Dr. Maren Eilísdóttir-Garapetyan / `@isogloss_atlantic`)

ライブ `data.js` を `.wf_langmap_query.mjs` で、開発チーム round 12「兄弟整合性監査」が **FIX 0 / KEEP 全件** と裁定した18セル相当を全件再照合し、加えて担当9言語（lt, lv, ga, cy, br, el, sq, hy, eu）× 全100文を対象に、複合ラベル内 role-letter 共有を含む**隣接同 role-letter の機械走査**を実施した。

### 機械走査の結果（最重要）

- 全100文 × 9言語の隣接同 role-letter 走査ヒット = **0 件**。複合ラベル（`B|C` `A|D|E` 等）内の文字も含めて隣接共有を判定したが、`feedback_no_adjacent_same_segments.md` 規約違反は1件も存在しない。開発側が round 12 で挙げた18セルの重複は**いずれも他セグメントを挟んだ非隣接の「同一役割分割」**であり、隣接違反ではないことをバイト照合で確認した。

### 開発チーム round 12 の KEEP 裁定（項目別・ライブ＋en参照照合）

1. **Welsh 迂言動詞/コピュラ（#42/#51/#56/#57/#59/#70/#73/#77/#92/#87/#91/#40）→ ✓ 全件受理**。ライブで助動詞（Mae / Mae'r / Rydyn / Cafodd / Ydych）と語彙動詞（yn+動詞名詞）が同一動詞役色を担い、両半は主語(A)等を挟んで非隣接。例 #42 `C:Mae A:hi B:bob amser C:yn gwisgo`／#77 `C:Cafodd A:e C:ei synnu`（受動迂言 got+surprised、en `C:was surprised` に整合）／#40 `E:Ydych A:chi'n E:gwybod`（**en 自身が `E:Do A:you E:know` と E を分割**しており明示的に正当）。兄弟言語に独立コピュラセグメントが無いため別文字へ retag すると逆に参照非整合になるという開発側の説明は妥当。ハウススタイル（#5 `D:Mat C:kenañ D:eo`）とも一致。
2. **Albanian 限定詞+形容詞の名詞周り分割（#11/#35/#65）→ ✓ 全件受理**。ライブ＋en 照合: #11 `D:një E:libër D:të ri`（en `D:a new`）／#35 `E:Vëllai A:im E:i madh`（en `E:older brother`、A=im を挟み非隣接）／#65 `C:një D:dhuratë C:të bukur`（en `C:a beautiful`）。en が単一文字で束ねる限定詞+形容詞を、sq が後置形容詞のため名詞を挟んで分割しており、参照 role と整合する正当な分割。
3. **Irish 迂言動詞（#93）→ ✓ 受理**。`G:bíonn … F:féilte G:ar siúl`（bíonn…ar siúl で are held を構成、en `G:are held` に整合）。F を挟み非隣接。妥当。

### round 1–10 受理済み項目の回帰検査

ライブ再照合で全て維持を確認、回帰なし:
- 語彙・正書法・非文: #5 br `D:Mat C:kenañ D:eo A:ar B:kafe-mañ`／#4 br `A:an tostañ B:ti-gar`／#36 br `C:er gar`／#97 br `C:er gar`（savañ なし）／#1 br `D:ur gwiskamant`／#1 hy `C:փորձել`／#16 hy `A:Քանիսին`／#84 hy `B|C:մրսեց`／#91 hy `A:Բալի ծաղկման դիտումը`／#12 hy `C:բալենի ծաղիկները`／#91 ga `A:féachaint ar bhláthanna silíní`（空白+lenition）／#12 ga `C:na bláthanna silíní`（空白）／#26 el `E:πάρκο`（先頭スペースなし）／#57 br `C:ampar`。
- SPLIT 群（round 4/9）および #84 eu `B:hotzeria C:harrapatu zuen`（隣接 C-C 解消後）も全て維持。
...
- なし。round 12 が挙げた18セルは形式上の重複検知だが、Celtic 迂言・Albanian 限定詞分割・Irish 迂言という言語固有の正当構造による「同一役割の非隣接分割」であり、いずれも en 参照 role と整合する。SPLIT 優先方針の観点でも、別文字への retag はかえって参照非整合を招くため不可。追加分割を要する誤った融合・隣接同色・claimed-but-missing はいずれも検出されなかった。

### スコアカード（round 13）

- 受理 ✓: 開発側 round 12 の KEEP 全18セル（cy 12・sq 3・ga 1 の同役割分割）＋ round 1–10 の語彙・正書法・非文・SPLIT 群（全て維持）。
- 部分 △ / 要対応: 0 件。
- 却下 ✗: 0 件。
- 私の撤回（CONCEDE）累計: 4 件（#4 役割割当案・#31 語順・#88 略語・br #43 D 切り出し）。round 13 新規 CONCEDE: 0 件。
- 検証で claimed-but-missing だった修正: 0 件（round 12 の KEEP 主張は全てライブで一致確認）。
- 全100文 × 9言語の隣接同 role-letter 走査ヒット: 0 件。
- 新規 actionable: 0 件。

開発側 round 12 の「FIX 0 / KEEP 全件」裁定はライブデータおよび en 参照と完全に整合しており、機械走査でも規約違反ゼロ、round 1–10 の全修正にも回帰なし。actionable な未修正項目・claimed-but-missing・新規の客観誤りはいずれも存在しない。
