...
## レビュアー再評価 — round 10 (@nordweich)

開発チームの round 9 包括スイープ（SPLIT 12 / KEEP 16）を、`node .wf_langmap_query.mjs <id> <code>` で**全件ライブ照合**しました。round 9 は「APPLIED」明示こそ無いものの、提案された 12 SPLIT がすべて実データに反映済みであることを確認しました（虚偽申告ゼロ）。

### A. SPLIT 12件 — 全件ライブ反映確認

- **#46 sv** ✓ `H:har E:pratat C:i telefon`。融合内に埋没していた C「電話で」を復元。3分割で C 役が立った。
- **#46 no** ✓ `H:har E:snakket C:i telefonen`。同上、C 復元。
- **#46 yi** ✓ `H:האָב E:גערעדט`。完了助動詞+過去分詞の2トークン分離。なお yi の C「אויפֿן טעלעפֿאָן」は元から独立しており復元不要だった点も確認。
- **#69 yi** ✓ `E:האָט D:געשריבן`。完了の助動詞(E=時制)+過去分詞(D=動詞)を分離。隣接同一なし。
- **#73 da** ✓ `E:Min A:kat`。所有+名詞分離（sv 既分離に合わせた fix-all）。
- **#73 no** ✓ `A:Katten E:min`（後置所有）。
- **#73 nn** ✓ `A:Katten E:min`。
- **#73 is** ✓ `A:Kötturinn E:minn`。
- **#73 fo** ✓ `A:Ketta E:mín`。
- **#73 yi** ✓ `E:מײַן A:קאַץ`（前置所有）。
- **#84 nn** ✓ `E:førre D:veke`（last+week 分離、sv の E/D 分割に合わせた fix-all）。
- **#84 fo** ✓ `E:seinastu D:vikuna`。
- **#84 yi** ✓ `E:פֿאַרגאַנגענע D:וואָך`。

12件すべて、異なる role-letter を与え隣接同一 letter を生まない正当な split であり、同グループ内で既分割の言語（sv/no #46、sv #73/#84）を手本とした fix-all 適用として一貫しています。オーナーの FINER-SEGMENTATION 方針に合致。

### B. KEEP 16件 — typology 上の妥当性を再検証し全件支持

ライブで複数を抜き取り照合し、いずれも分離可能形態素を持たない／分割すると役無しトークンが残るため KEEP が正しいと確認:

- **#34 比較級副詞**（långsammare 等）= もっと(B)+ゆっくり(D)が単一比較級に融合、語境界なし。fo のみ meira+seinliga で既分割。✓ KEEP 妥当。
- **#38 指示副詞**（härifrån/héðan/hiðani 等）= here+from の融合副詞1トークン。yi は פֿון+דאַנען で既分割。✓
- **#46 da/nn/is/fo E|H「talte/snakka/talaði/tosaði」** = 総合的単語過去（語根+過去）1トークン。ライブで C「i telefon/på telefonen/í síma」が**既に独立**していることも確認 — sv/no の C 復元は periphrastic 完了形ゆえの粒度差で、da 系の C は元から立っている。グループ内整合済み。✓
- **#52 C|H da/nn/is/fo**（mistede/mista/týndi/misti）= 総合的単語過去。sv/no/yi は har+分詞で既分割。✓
- **#53 C|E**（blommar 等）= 咲く(C)+進行(E)の単一現在形。北ゲルマン/yi に進行助動詞なし。✓
- **#55 is B|F「kaffibolli」** = kaffi+bolli の複合名詞1語。✓
- **#69 D|E sv/da/no/nn/is/fo**（skrev/skreiv/skrifaði）= 総合的単語過去。yi のみ完了迂言形で分割。✓
- **#77 B|E**（provresultaten 等、全6言語）= テスト(E)+結果(B)の複合名詞1語。yi のみ分かち書きで2語分割。ライブ確認済み。✓
- **#84 is B|C「kvefaðist」** = 「風邪をひく」の総合的単一再帰動詞。✓
- **#84 da D|E「i sidste uge」/is D|E「í síðustu viku」** = 前置詞 i/í が孤立（en「last week」に前置詞役なし、sv も `E:förra D:veckan` と前置詞なし）。E/D に割ると i が役無しで残るため非分割。**この KEEP 理由は妥当** — ライブで sv が前置詞を持たないことを確認し、da/is の i/í は割当先の役が存在しないことを裏付けた。✓
- **#96 D|E**（nästa 等）= 表層「次の(E)」単一語のみ、冠詞 D は北ゲルマン語で類型的に非実現。yi のみ דער+קומענדיקער で冠詞あり既分割。ライブ確認済み。✓

KEEP 群はいずれも (a) 単一語化（比較級副詞・指示副詞・複合名詞）、(b) 総合的活用（単語過去・再帰動詞）、(c) 冠詞非実現、(d) 割当先の役が無い前置詞、のいずれかに該当し、分割は不可能か役無しトークン／隣接同一 letter を生みます。SPLIT 優先方針の下でも KEEP が正当です。全件支持。

### 新規 issue

0件。round 9 のスイープは群14の全複合セルを網羅しており、抜き取り照合の範囲で分離可能な前置詞・冠詞・形容詞・量化詞・所有詞・助動詞・主語代名詞はすべて独立セグメント化済み、残る融合はすべて typology 上分離不可と確認しました。padding 目的の新規指摘はしません。

### 既往 CONCEDE（再確認）

#28 עמפֿעלן（語彙選好）、#15 utomlands（編集裁量）、#19 sv gärna（規約・de 前例あり）、#50 sv gärna/tycker om（de #50 と同型）、#45/#93 yi דאָזיק（指示詞=語義追加の編集裁量）、#57 fo rindliga→flótandi（辞書照合ツール不在で双方確証不可＝既知の限界）。計6件、いずれも本ラウンドでも維持（取り下げ済みのまま）。

### スコアカード（round 10）

- 検証した round 9 SPLIT: **12件すべてライブ反映済み・正当 (✓)**。虚偽申告ゼロ。
- KEEP 16件: 全件 typology 上妥当として支持。
- 本ラウンドでの追加 CONCEDE: 0件（既往 CONCEDE 6件は維持）。
- 新規 issue: 0件。
- 残存 actionable: **なし**。

round 1〜9 を通じ、fo の正書法・語彙誤り、yi の文法・性一致・補文標識、北欧語の主語分割（#95）、全グループの分割粒度（後置所有・3役融合・完了迂言形・所有+名詞・last+week・電話句 C 復元）がすべて解消・検証済みです。残るのはオーナーが編集裁量／既知の限界と認める語彙選好（#57 rindliga、yi #45 דאָזיק）のみで、いずれも concretely actionable な統語エラーではありません。ループ終了条件に従い CLOSE します。

**ファイル状態: CLOSED — 残課題なし**
（既知の限界: #57 fo「rindliga→flótandi」は辞書照合ツール不在のため双方確証できず保留／yi #45 の指示詞 דאָזיק 補充は語義追加の編集裁量として won't-fix。いずれも統語エラーではない。）

## 兄弟整合性監査 — 群14 (作業者 round 12)

外れ値として旗揚げされた4セルをライブ照会し、兄弟整列＋en/ja参照と照合した。結論: 全件 KEEP（誤割当なし）。いずれも「迂言的動詞（助動詞+本動詞/分詞）」または「役割分割（D要素を挟んで同役割が2分割）」という、当該言語の正当な構造であり、参照(en)または言語類型がこれを支持する。

### #11 — [yi] C 重複 → KEEP
ライブ: F:נעכטן C:האָב A:איך D:אַ נײַ E:בוך C:געקויפֿט B:אין בוכהאַנדלונג
理由: yi は単純過去を欠き完了迂言形のみ。「האָב…געקויפֿט」=「have…bought」は助動詞+過去分詞で一つの動詞(=bought)。兄弟(sv köpte / is keypti 等)は単純過去なので単一C、yi は構造上2分割せざるを得ず両方とも正しく C。完了迂言形は本群で既出のKEEPパターン。

### #21 — [yi] E 重複 → KEEP
ライブ: B:הײַנט בײַנאַכט E:וועל A:איך D:וועטשערע E:עסן C:מיט פֿרײַנד
理由: 「וועל…עסן」=「will…eat」は未来助動詞+本動詞の迂言形で一つの動詞(=will have)。en も E:「will have」を単一Eとするが yi は語順上 D(目的語)を挟んで2分割。da「skal spise」/nn「skal ete」/is「mun borða」と同種の助動詞+本動詞構造で、yi のみ語順により分離するため両方 E が正しい。

### #42 — [da] E 重複 → KEEP
ライブ: A:Hun C:bærer B:altid E:en D:rød E:kjole
理由: 参照 en 自身が E:「a」…D:「red」…E:「dress」と、限定詞(E)が形容詞(D)を挟んで名詞(E)と2分割される構造。da「en…rød…kjole」はこれを正確に鏡写し(E/D/E)。後置形容詞を挟む限定詞複合の正当な分割で、ブリーフ明記のKEEP事例。

### #50 — [sv] C 重複 → KEEP
ライブ: E:Min A:hund C:går D:gärna på C:promenad
理由: en の C:「to go for walks」に相当する動詞句を、sv は「går…på promenad」(=go…for a walk)と D要素(gärna på=好んで/likes)を挟んで2分割。「går」と「promenad」は同一の散歩行為(役割C)を構成する分離動詞句で、間に挟まる D により分割されるため両方 C が正しい。

## レビュアー再評価 — round 13 (@nordweich)

開発チームの round 12（兄弟整合性監査：外れ値4セルを全件 KEEP と判定）を、`node .wf_langmap_query.mjs <id> <code>` で**全件ライブ照合**しました。提示された4セルのライブ値はすべて回答の引用と一致（虚偽申告ゼロ）。さらに各 KEEP 理由の根拠となる参照言語（en/de）および兄弟言語を直接照会し、類型的妥当性を独立に検証しました。

### 4セルの裁定（全件 KEEP を支持）

- **#11 yi C 重複（האָב…געקויפֿט）— ✓ ACCEPT KEEP**
  ライブ `F:נעכטן C:האָב A:איך D:אַ נײַ E:בוך C:געקויפֿט B:אין בוכהאַנדלונג`（回答と一致）。yi は単純過去を欠き完了迂言形（助動詞 האָב + 過去分詞 געקויפֿט）のみで「bought」を表す。兄弟をライブ照会したところ sv `C:köpte`・is `C:keypti` はいずれも単純過去＝単一 C で、yi は V2 倒置＋目的語介在のため構造上2分割せざるを得ない。両 C はともに同一動詞役を構成し、間に A/D/E を挟むため隣接同一 letter にもならない。完了迂言形の2分割は本群で round 9（#46/#52/#69）以来確立済みの正当パターン。**分割は不可能（単一動詞役の語順分散であり融合ではない）。KEEP 正当。**

- **#21 yi E 重複（וועל…עסן）— ✓ ACCEPT KEEP**
  ライブ `B:הײַנט בײַנאַכט E:וועל A:איך D:וועטשערע E:עסן C:מיט פֿרײַנד`（一致）。**参照 en 自身が `E:will have` を単一 E** とし、兄弟 da `E:skal spise`・nn `E:skal ete`・is `E:mun borða` はいずれも助動詞+本動詞を単一 E に融合。yi のみ V2 倒置で主語 A・目的語 D を挟むため、未来助動詞 וועל と本動詞 עסן が同一役 E のまま2分割される。役割 E のカバレッジは保たれ、これは語順差に起因する分散であって誤割当ではない。**KEEP 正当。**

- **#42 da E 重複（en…rød…kjole）— ✓ ACCEPT KEEP**
  ライブ `A:Hun C:bærer B:altid E:en D:rød E:kjole`（一致）。**参照 en 自身がモデル**で `E:a D:red E:dress` と、限定詞(E)が形容詞(D)を挟んで名詞(E)と2分割される構造。da はこれを正確に鏡写し（E/D/E）。限定詞複合の正当な分割であり、両 E は同一名詞句役。de #50 型と同じく「ブリーフ明記の許容パターン」。**KEEP 正当。**

- **#50 sv C 重複（går…på promenad）— ✓ ACCEPT KEEP（既 CONCEDE の再確認）**
  ライブ `E:Min A:hund C:går D:gärna på C:promenad`（一致）。**参照 de #50 `C:geht D:gerne C:spazieren` と完全同型**で、これは round 2 ですでに CONCEDE 済みの確立規約。D（gärna på）を挟んで動詞句役 C が2分割されるのは意図的・一貫した構造で、非隣接 C-C は隣接同一 letter 規約に非抵触。**KEEP 正当。**

### 検証の総括

4件はいずれも (a) 完了/未来の迂言的動詞（助動詞+本動詞/分詞）が V2 語順で介在要素により分散したもの（#11/#21）、または (b) 限定詞複合・動詞句が形容詞/副詞を挟んで分散したもの（#42/#50）で、**いずれも参照言語（en が #21/#42 で、de が #50 で）自身が同パターンを採用**しているか、兄弟言語の単一役と整合する。分割すれば同一役を別 letter に割る根拠がなく（同一の動詞役・名詞句役だから）、SPLIT 優先方針の対象外。すなわち「分離可能な別役の融合」ではなく「単一役の語順分散」であり、KEEP が正しい。

### 新規 issue

0件。round 12 の兄弟整合性監査は外れ値4セルを正確に処理しており、各 KEEP は参照/類型で裏付け済み。padding 目的の新規指摘はしない。round 1〜10 で解消・検証済みの fo 正書法・語彙（tosaði/lesi/kedd/Konsertin/mánaðarenda/Læknarin/vegin/Sólsetrið）、yi 文法・性一致・補文標識（די סביבה/געכאַפּט/די דאָזיקע/אַז）、北欧語主語分割（#95）、全グループ分割粒度（後置所有・3役融合・完了迂言形・所有+名詞・last+week・電話句 C 復元）の代表セルを本ラウンドでも抜き取りライブ照合し、いずれも反映維持を確認した。

### 既往 CONCEDE（再確認・維持）

#28 עמפֿעלן（語彙選好）、#15 utomlands（編集裁量）、#19 sv gärna（規約・de 前例）、#50 sv gärna/tycker om（de #50 同型）、#45/#93 yi דאָזיק（語義追加の編集裁量）、#57 fo rindliga→flótandi（辞書照合ツール不在で双方確証不可＝既知の限界）。計6件、いずれも維持（取り下げ済みのまま）。

### スコアカード（round 13）

- 検証した round 12 KEEP: **4件すべてライブ一致・類型的に正当 (✓)**。虚偽申告ゼロ。
- 本ラウンドでの追加 CONCEDE: 0件（既往 CONCEDE 6件は維持）。
- 新規 issue: 0件。
- 残存 actionable: **なし**。

round 1〜12 を通じ、確実度の高い fo 正書法・語彙誤り、yi 文法・性一致・補文標識、北欧語の主語分割、全グループの分割粒度（後置所有・前置詞句・3役融合・完了迂言形）がすべて解消・検証済みです。round 12 で扱われた外れ値4セルは、参照言語自身が採用するか兄弟言語の単一役と整合する「単一役の語順分散」であり、分割対象ではありません。残るのはオーナーが編集裁量／既知の限界と認める語彙選好（#57 rindliga、yi #45 דאָזיק）のみで、いずれも concretely actionable な統語エラーではありません。ループ終了条件に従い CLOSE します。
