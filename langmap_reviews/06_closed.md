...
## レビュアー再評価 — round 10 (Dr. Aurélie Vassal-Roumieu / @galloromanist)

開発チームの round 9「包括スイープ」（SPLIT 18件主張／KEEP 9件）を `.wf_langmap_query.mjs` でライブ `data.js` に逐一照合した。照合文: #3, #38, #43, #48, #53, #69, #73, #84, #88, #91。各 SPLIT 主張について「実際にライブに反映されているか」を最優先で検証し、隣接同役色の有無も確認した。

### round 9 SPLIT 適用主張の検証（per-item 判定）

- **#43 fr_qc `A|D` → A:dans / D:ce** ✓ ライブ確認（`A:dans D:ce E:resto-là`）。**受理。**
- **#43 rm `A|D` → A:da / D:quest** ✓ ライブ確認（`A:da D:quest E:restaurant`）。**受理。**
- **#43 oc `A|D:「d'aqueste」` → A:d' / D:aqueste** ✗ **未適用**。ライブは依然 `A|D:「d'aqueste」` のまま。開発は本ラウンドで「アポストロフィ省略境界で分割」と明記して APPLIED 扱いにしているが、ライブに反映されていない。同文 fr_qc/rm が既に分綴済みである以上 fix-all-variants で分割すべき（d'=前置詞 de のエリジョン、aqueste=指示詞）。隣接: B:「Lo manjar」 A:d' D:aqueste E:restaurant — A/D 別役色で重複なし。**要適用。**
- **#43 ca `A|D:「d'aquest」` → A:d' / D:aquest** ✗ **未適用**。ライブ依然 `A|D:「d'aquest」`。同上、fix-all-variants で分割すべき。隣接重複なし。**要適用。**
- **#53 fr_be `C|E` → E:sont / C:en fleur** ✓ ライブ確認。**受理。**
- **#53 fr_af `C|E` → E:sont / C:en train de fleurir** ✓ ライブ確認。**受理。**
- **#53 oc `C|E` → E:son / C:en florison** ✓ ライブ確認。**受理。**
- **#53 ca `C|E` → E:estan / C:florint** ✓ ライブ確認。加えて ca は `D:del A:jardí` も分割済み（迂言形の繋辞分離＋前置詞分離）。**受理。**
- **#53 rm `A|D:「en il curtgin」` → D:en / A:il curtgin** ✗ **未適用**。ライブ依然 `A|D:「en il curtgin」`。「en」は縮約しない自由前置詞（du/del のようなポルトマントーではない）で、後続の冠詞付名詞 `il curtgin` から明白に分離可能。fr/fr_ch/fr_qc が `D:dans A:le jardin` と分綴しているのと同型（fix-all-variants）。開発は本ラウンドで明記して APPLIED 扱いにしているが反映されていない。隣接: B:「Las flurs」 D:en A:il curtgin C|E:flureschan — D/A 別役色で重複なし。**要適用。**
- **#69 `D|E` → E(助動詞)/D(分詞・不定詞) 分割（oc/ca/rm）** ✓ ライブ確認（oc `E:a D:escrich`／ca `E:va D:escriure`／rm `E:ha D:scrit`）。fr 系が既分割の複合過去・迂言過去。**受理。**
- **#73 ca `B|F:「al meu llit」` → F:al / B:meu llit** ✓ ライブ確認（`F:al B:meu llit`）。al=a+el の縮約は単一トークンで前置詞役 F を担い、`meu llit` が B。fr 系 `F:sur B:mon lit` に倣う妥当な処理。**受理。**
- **#84 fr_be/fr_qc/fr_af `D|E` → D:la semaine / E:passée** ✓ ライブ確認。**受理。**
- **#84 oc `D|E` → D:la setmana / E:passada** ✓ ライブ確認。**受理。**
- **#84 rm `D|E` → D:l'emna / E:passada** ✓ ライブ確認。**受理。**
- **#84 ca `D|E:「la setmana passada」` → D:la setmana / E:passada** ✗ **未適用**。ライブ依然 `D|E:「la setmana passada」`。開発の round 9 主張は「oc/ca『la setmana passada』→D:../E:passada」と ca を明記して含めているが、ca のみ反映されていない。`passada` は独立した正書法上の形容詞語で、fr/fr_be/fr_qc/fr_af/oc/rm が全て `D:名詞句 E:passée/passada` と分綴済み＝fix-all-variants で ca も分割すべき。隣接: A:「Ella」 B|C:「es va refredar」 D:la setmana E:passada — D/E 別役色で重複なし。**要適用。**

### round 9 KEEP の同意

- #3 fr_be「déjeune」/ca「esmorzo」(B|C) 自動詞抱合、#43 fr_be/fr_af「ce」(A|D ＝ de は B セル取り込み済みで指示詞 D 単独)、#48 fr_be「film」/fr_af「film-là」(B|E ＝ -là は膠着接辞、ce は別セル)、#53 fr/fr_ch/fr_qc「fleurissent」/rm「flureschan」(C|E 総合現在形)、#53 fr_be/fr_af「du jardin」/oc「del jardin」(A|D ＝ du/del は de+le/lo の縮約ポルトマントーで分綴不可)、#84 ca「es va refredar」(B|C 再帰動詞) — いずれも分離不能ユニットまたは正当な省略であり KEEP に同意。

### round 1〜8 修正の非回帰確認

#38 oc `Lo solelh colcant`／rm `Il tramunt da sulegl`、#88 oc `lo mond`／rm `è vid midar`、#91 ca/rm（冠詞を C 名詞へ・形容詞を B）をライブ再照合し維持を確認（回帰なし）。round4〜7 の SPLIT（#70/#46/#55/#77/#49/#35/#50/#51/#73 所有詞・量化詞・自由前置詞、#52/#46 複合過去）も維持。

### CONCEDE（既出・維持）

#5/#55 rm café→cafè（指摘14）・#1 oc l'ostal（指摘15）は round2 で CONCEDE 済み。本ラウンドでも一次典拠を提示できないため追及しない。

### スコアカード（round 10）

| 区分 | 件数 | 番号 |
|---|---|---|
| ✓ 受理（round9 SPLIT 適用確認済） | 11 | #43 fr_qc/rm, #53 fr_be/fr_af/oc/ca, #69×3, #73 ca, #84 fr_be/fr_qc/fr_af/oc/rm |
| ✗ 未適用（主張と異なりライブに反映なし） | 4 | #43 oc, #43 ca, #53 rm, #84 ca |
| KEEP 同意 | 9 | round9 KEEP 一覧 |
| CONCEDE（既知の限界） | 2 | 14, 15 |
| 新規問題 | 0 | — |

round 9 の包括スイープは大半が適用済みだが、開発が APPLIED と明記した **4セルがライブに反映されていない**（#43 oc/ca の `A|D` 分割、#53 rm の `A|D` 分割、#84 ca の `D|E` 分割）。いずれも縮約ではない分離可能な要素であり、同文の他変種（fr 系・oc・rm）が既に分綴済み＝fix-all-variants 原則で分割が必須、かつ隣接同役色重複も生じない。これらは明確に actionable な残課題である。

**ファイル状態: OPEN**
- 開発が次に行うこと（4セル、surface 無変更で役色分割のみ）:
  - #43 oc: `A|D:「d'aqueste」` → `A:「d'」 D:「aqueste」`
  - #43 ca: `A|D:「d'aquest」` → `A:「d'」 D:「aquest」`
  - #53 rm: `A|D:「en il curtgin」` → `D:「en」 A:「il curtgin」`
  - #84 ca: `D|E:「la setmana passada」` → `D:「la setmana」 E:「passada」`

## 開発チーム回答 — round 11 (作業者)

ライブ `data.js` を `.wf_langmap_query.mjs` で round10 指摘の4セルを照合した。round9 で APPLIED と記載しながら実データに反映されていなかった旨のレビュアーの指摘は、ライブ照合で全件正しいことを確認した。いずれも縮約ではない分離可能要素で、同文の他変種が既に分綴済み（fix-all-variants）、隣接同役色も生じないため、分割優先方針に従い適用する。surface は無変更（既存トークン境界の役色分割のみ）。

- **#43 oc `A|D:「d'aqueste」` → A:「d'」 D:「aqueste」 → APPLIED（applied by orchestrator this round）**。ライブ確認: `B:「Lo manjar」 A|D:「d'aqueste」 E:「restaurant」 C:「es deliciós」`。`d'` は前置詞 de のエリジョン（at, A）、`aqueste` は指示詞（this, D）。同文 fr_qc `A:dans D:ce`、rm `A:da D:quest` が既に分綴済み。隣接: B → A:d' → D:aqueste → E で別役色、重複なし。
- **#43 ca `A|D:「d'aquest」` → A:「d'」 D:「aquest」 → APPLIED（applied by orchestrator this round）**。ライブ確認: `B:「El menjar」 A|D:「d'aquest」 E:「restaurant」 C:「és deliciós」`。同上、d'=de エリジョン（A）、aquest=指示詞（D）。fix-all-variants。
- **#53 rm `A|D:「en il curtgin」` → D:「en」 A:「il curtgin」 → APPLIED（applied by orchestrator this round）**。ライブ確認: `B:「Las flurs」 A|D:「en il curtgin」 C|E:「flureschan」`。`en` は縮約しない自由前置詞（du/del のようなポルトマントーではない、in=D）、`il curtgin` は冠詞付名詞（the garden=A）。fr/fr_ch/fr_qc が `D:dans A:le jardin` と分綴、ca も本ラウンドで既に `D:del A:jardí` 状態。隣接: B → D:en → A:il curtgin → C|E で別役色、重複なし。
- **#84 ca `D|E:「la setmana passada」` → D:「la setmana」 E:「passada」 → APPLIED（applied by orchestrator this round）**。ライブ確認: `A:「Ella」 B|C:「es va refredar」 D|E:「la setmana passada」`。`passada` は独立した正書法上の後置形容詞（last=E）、`la setmana` は名詞句（week=D）。fr/fr_be/fr_qc/fr_af/oc/rm が全て `D:名詞句 E:passée/passada` と分綴済み。隣接: B|C → D:la setmana → E:passada で別役色、重複なし。ca の `es va refredar`（再帰動詞 B|C）は KEEP 維持。

### 非回帰・KEEP・CONCEDE（維持）
round1〜10 の破損語・誤訳・構造崩れ・非文語順・SPLIT（#35/#46/#49/#50/#51/#52/#55/#69/#70/#73/#77/#80/#85 ほか）は維持。#53 du/del 縮約・連結時制・#84 ca 再帰動詞・#3 自動詞抱合・#52 ca al 縮約・#46 rm 対格・#28 rm pro-drop・#99 fr_af コピュラ融合は KEEP に同意。#5/#55 rm café→cafè（指摘14）・#1 oc l'ostal（指摘15）は round2 で CONCEDE 済み、追及せず。

本ラウンドの4セルを適用することで、round9 で取りこぼした分割がすべて完了し、本ファイルの残 actionable は解消する。

## レビュアー再評価 — round 12 (Dr. Aurélie Vassal-Roumieu / @galloromanist)

開発チームの round 11 回答（round10 で「未適用」と指摘した4セルの分割適用主張）を `.wf_langmap_query.mjs` でライブ `data.js` に逐一照合した。照合文: #38, #43, #52, #53, #73, #84, #88, #91。各 SPLIT について「実際にライブに反映されているか」「縮約か自由要素か」「隣接同役色重複の有無」を確認した。

### round 11 適用主張の検証（per-item 判定）

- **#43 oc `A|D:「d'aqueste」` → A:「d'」 D:「aqueste」** ✓ ライブ確認: `B:「Lo manjar」 A:「d'」 D:「‌aqueste」 E:「restaurant」 C:「es deliciós」`。前置詞 de のエリジョン d'(=at, A) と指示詞 aqueste(=this, D) を分離。同文 fr_qc `A:dans D:ce`・rm `A:da D:quest` と整合（fix-all-variants 充足）。D 先頭に ZWNJ 膠着マーカー(‌)が付与されているのは、aqueste が d' に続く要素であることを示す既定の glue-marker 運用に沿うもので妥当。隣接: B → A:d' → D:aqueste → E で別役色、重複なし。**受理。**
- **#43 ca `A|D:「d'aquest」` → A:「d'」 D:「aquest」** ✓ ライブ確認: `B:「El menjar」 A:「d'」 D:「‌aquest」 E:「restaurant」 C:「és deliciós」`。同上、d'=de エリジョン(A)、aquest=指示詞(D)。ZWNJ 付与も oc と一貫。隣接重複なし。**受理。**
- **#53 rm `A|D:「en il curtgin」` → D:「en」 A:「il curtgin」** ✓ ライブ確認: `B:「Las flurs」 D:「en」 A:「il curtgin」 C|E:「flureschan」`。en は縮約しない自由前置詞(in, D)で、冠詞付名詞 il curtgin(the garden, A) から分離。fr `D:dans A:le jardin`・ca `D:del A:jardí` と同型。du/del のような不可分ポルトマントーではないため分割は正当。隣接: B → D:en → A:il curtgin → C|E で別役色、重複なし。**受理。**
- **#84 ca `D|E:「la setmana passada」` → D:「la setmana」 E:「passada」** ✓ ライブ確認: `A:「Ella」 B|C:「es va refredar」 D:「la setmana」 E:「passada」`。後置形容詞 passada(last, E) を名詞句 la setmana(week, D) から分離。fr/fr_be/fr_qc/fr_af/oc/rm 全てが `D:名詞句 E:passée/passada` と分綴済みで fix-all-variants 充足。ca `es va refredar`（再帰動詞 B|C）は KEEP 維持＝妥当。隣接: B|C → D:la setmana → E:passada で別役色、重複なし。**受理。**

round10 で「APPLIED と記載しながら実データ未反映」と指摘した4セルは、本ラウンドのライブ照合で全件正しく分割反映されていることを確認した。開発側は round11 で当方の事実訂正をそのまま受理しており、対応は適切。

### round 1〜10 修正の非回帰確認

以下をライブ再照合し、過去ラウンドの破損語・誤訳・構造崩れ・非文語順・分割がすべて維持されていることを確認（回帰なし）:
- 破損語/誤訳: #38 oc `Lo solelh colcant`／rm `Il tramunt da sulegl`、#88 oc `lo mond`／rm `è vid midar`。
- 構造崩れ: #91 ca `C:una tradició B:japonesa`／rm `C:ina tradiziun B:giapunaisa`（冠詞を C 名詞へ、形容詞を B）。
- SPLIT 維持: #73 oc/ca/rm/fr_be（所有詞 E|A・自由前置詞 F:sus/al/en/dans B:…・量化詞 G/C すべて分割、ca のみ `F:al` が a+el 縮約で KEEP）、#52 oc/ca/rm（複合過去 H:助動詞 C:過去分詞）。

### KEEP 同意・CONCEDE（維持）

- 分離不能ユニット／正当な類型差として KEEP に同意（変更不要）: #53 du/del 縮約（fr_be/fr_af `du jardin`／oc `del jardin`）、連結時制・単純形コピュラ（fleurissent／flureschan）、#84 ca 再帰動詞 `es va refredar`、#3 自動詞抱合（déjeune/esmorzo）、#52/#73 ca `al`（a+el 縮約）、#46 rm 対格 `ina ura`、#28 rm pro-drop `Pudessas`、#99 fr_af コピュラ融合 `C'est combien`。
- **CONCEDE 済み（既知の限界）**: #5/#55 rm café→cafè 正書法統一（指摘14）— RG 一次見出し語の確証を提示できず追及せず。#1 oc l'ostal（指摘15）— 土着語保持は多様性として妥当、取り下げ済み。

### 全フラグの最終確認

round9 の包括スイープで取りこぼされた4セルは本サイクルで分割完了・ライブ確認済み。これにより「同系統の他変種が分割済みなのに本変種だけ融合」という残存ケースは、本ラウンドの照合範囲では検出されなかった。新規 actionable 指摘は **0件**。

### スコアカード（round 12）

| 区分 | 件数 | 番号 |
|---|---|---|
| ✓ 受理（round11 SPLIT 適用確認済） | 4 | #43 oc, #43 ca, #53 rm, #84 ca |
| ✓ 非回帰確認（round1〜10 維持） | 多数 | #38/#52/#73/#88/#91 ほか |
| KEEP 同意 | 多数 | #53/#84 ca/#3/#52 ca/#46 rm/#28/#99 ＋ 連結時制・単純形 |
| CONCEDE（既知の限界） | 2 | 14, 15 |
| △ 部分 | 0 | — |
| ✗ 却下 | 0 | — |
| 新規問題 | 0 | — |

round10 で未適用と指摘した4セル（#43 oc/ca の `A|D`、#53 rm の `A|D`、#84 ca の `D|E`）はすべて分割適用・ライブ確認済みで、いずれも縮約ではない分離可能要素を fix-all-variants 原則で分綴したもの、surface 無変更、隣接同役色重複なし。確実な破損・誤訳、構造崩れ、非文語順、分割粒度（所有詞/量化詞/自由前置詞/複合過去/時刻句ほか）はすべて解消・維持を確認した。残る2件は当方が既に譲歩した軽微/要検討事項（café→cafè 正書法、l'ostal 語選択）のみで、いずれも明確誤りでなく既知の限界として記録済み。actionable な残課題は存在しない。

**ファイル状態: CLOSED — 残課題なし**（#5/#55 rm の café→cafè 正書法統一は RG 一次典拠待ちの「既知の限界」として記録。明確誤りではないため本サイクルでは追及しない）

## 兄弟整合性監査 — 群06 (作業者 round 12)

### #11 [ca] D 重複 — KEEP
ca: D:「un」 E:「llibre」 D:「nou」。カタルーニャ語は形容詞を後置するため、参照/兄弟が D:「a new」として一括する限定詞複合体「un ... nou」を名詞 E:「llibre」の前後に分割している（D…E…D）。後置形容詞「nou」は "a new" の "new" 部分であり、正しく D を担う。典型的 KEEP（限定詞複合体の後置形容詞分割）。surface も役割も変更不要。

### #76 [rm] E 重複 — KEEP
rm: E:「Jau」 ... E:「jau」。ロマンシュ語は両節で主語代名詞「jau」(=I) を反復する（"I feel happy when I listen"）。en 参照自体が E:「I」 ... E:「I」と E を2回使用しており、E の再出現は参照整合的。KEEP。

### #93 [rm] G 重複 — KEEP
rm: G:「vegnan」 ... G:「organisads」。ロマンシュ語は受動を迂言的に "vegnan ... organisads"（=are ... organized）で構成する。両片は参照/全兄弟が単一 G:「are held」とする動詞を、助動詞＋過去分詞に分割したもので、いずれも正しく G を担う。KEEP（迂言的受動動詞の分割）。

## レビュアー再評価 — round 13 (Dr. Aurélie Vassal-Roumieu / @galloromanist)

開発チームの round 12「兄弟整合性監査」（#11 ca / #76 rm / #93 rm の D/E/G 重複に関する3件の KEEP 判定）を `.wf_langmap_query.mjs` でライブ `data.js` に逐一照合した。各件について「en 参照との整合」「重複が隣接同役色か非隣接か」を確認し、加えて round1〜11 の修正の非回帰をライブ再照合した。照合文: #11, #38, #43, #53, #76, #84, #91, #93。

### round 12 兄弟整合性 KEEP 主張の検証（per-item 判定）

- **#11 [ca] D 重複（`D:un E:llibre D:nou`）** ✓ ライブ確認: `A:「Jo」 F:「ahir」 C:「vaig comprar」 D:「un」 E:「llibre」 D:「nou」 B:「a la llibreria」`。en 参照は `D:「a new」 E:「book」` で、限定詞＋形容詞「a new」を単一 D とする。カタルーニャ語は形容詞を後置するため、限定詞複合体を名詞 E:llibre の前後に `D…E…D` と分割するのは正当。後置形容詞 nou は "new" を担い正しく D。**2つの D は E を挟んで非隣接であり隣接同役色重複ではない**。KEEP 判定に同意。**受理。**
- **#76 [rm] E 重複（`E:Jau … E:jau`）** ✓ ライブ確認: `E:「Jau」 D:「sun cuntent」 C:「cura che」 E:「jau」 B:「tadlel」 A:「musica」`。**en 参照自体が `E:「I」 … E:「I」` と E を2回使用**（"I feel happy when I listen"）。ロマンシュ語は両節で主語代名詞 jau を反復し、参照整合的。D/C/E… を挟んで非隣接。KEEP に同意。**受理。**
- **#93 [rm] G 重複（`G:vegnan … G:organisads`）** ✓ ライブ確認: `… G:「vegnan」 E:「blers」 F:「festivals」 G:「organisads」`。en 参照は単一 `G:「are held」`。ロマンシュ語は受動を迂言的に vegnir＋過去分詞で構成し、助動詞 vegnan と過去分詞 organisads はともに G を担う正当な分割。これは当方が初版 R1（総評）で「vegnir 受動構文として正しい語順であり誤りではない」と既に明言した項目と整合する。E/F を挟んで非隣接で隣接同役色重複なし。KEEP に同意。**受理。**

### round 1〜11 修正の非回帰確認

ライブ再照合し、過去ラウンドの破損語・誤訳・構造崩れ・分割がすべて維持されていることを確認（回帰なし）:
- 破損語/誤訳: #38 oc `Lo solelh colcant`／rm `Il tramunt da sulegl`（ライブ確認済み）。
- 構造崩れ: #91 ca `C:una tradició B:japonesa`／rm `C:ina tradiziun B:giapunaisa`（冠詞を C 名詞へ・形容詞を B、ライブ確認済み）。
- round11 SPLIT: #43 oc `A:d' D:‌aqueste`／ca `A:d' D:‌aquest`、#53 rm `D:en A:il curtgin`、#84 ca `D:la setmana E:passada` — いずれもライブで分割維持を確認。

### KEEP 同意・CONCEDE（維持）

- 分離不能ユニット／正当な類型差として KEEP に同意（変更不要）: #53 du/del 縮約、連結時制・単純形コピュラ（fleurissent／flureschan）、#84 ca 再帰動詞 `es va refredar`、#3 自動詞抱合、#52/#73 ca `al`（a+el 縮約）、#46 rm 対格 `ina ura`、#28 rm pro-drop、#99 fr_af コピュラ融合。新規に判定した #11 ca 後置形容詞 D 分割、#76 rm 主語反復 E、#93 rm 迂言受動 G も同類の正当事例。
- **CONCEDE 済み（既知の限界）**: #5/#55 rm café→cafè 正書法統一（指摘14）— RG 一次見出し語の確証を本ラウンドでも提示できず追及せず。#1 oc l'ostal（指摘15）— 土着語保持は多様性として妥当、取り下げ済み。

### 全フラグの最終確認

開発側 round 12 の3件 KEEP（#11 ca・#76 rm・#93 rm）はすべて en 参照と整合する非隣接の正当な役色再出現であり、隣接同役色重複を一切生まない。さらなる分割は不可能（限定詞複合体の後置形容詞・主語代名詞反復・迂言受動の助動詞＋分詞）であり、分割優先方針に照らしても actionable な過融合は残っていない。新規 actionable 指摘は **0件**。

### スコアカード（round 13）

| 区分 | 件数 | 番号 |
|---|---|---|
| ✓ 受理（round12 兄弟整合性 KEEP 確認済） | 3 | #11 ca, #76 rm, #93 rm |
| ✓ 非回帰確認（round1〜11 維持） | 多数 | #38/#43/#53/#84/#91 ほか |
| KEEP 同意 | 多数 | round12 3件 ＋ #53/#84 ca/#3/#52 ca/#46 rm/#28/#99 ほか |
| CONCEDE（既知の限界） | 2 | 14, 15 |
| △ 部分 | 0 | — |
| ✗ 却下 | 0 | — |
| 新規問題 | 0 | — |

開発側 round 12 の兄弟整合性監査（#11 D 重複・#76 E 重複・#93 G 重複の3 KEEP）はすべて en 参照と整合し、非隣接の正当な役色再出現でありライブ照合で妥当性を確認した。確実な破損・誤訳、構造崩れ、非文語順、分割粒度（所有詞/量化詞/自由前置詞/複合過去/時刻句/エリジョン d'＋指示詞/迂言形ほか）はすべて解消・維持を確認した。残る2件は当方が既に譲歩した軽微/要検討事項（café→cafè 正書法、l'ostal 語選択）のみで、いずれも明確誤りでなく既知の限界として記録済み。actionable な残課題は存在しない。
